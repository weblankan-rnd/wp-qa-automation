  ...
  for (const pagePath of pagesToCheck) {
    ...
    test(`SEO checks on ${label} @smoke`, async ({ page }) => {
      ...
      await test.step('title length 10–70 chars', async () => {
        ...
        if (!title) { expect.soft(false, `[SEO] Missing <title> on ${pagePath}`).toBeTruthy(); return; }
        if (title.length < 10) expect.soft(false, `[SEO] Title too short on ${pagePath}: "${title}" (${title.length} chars, min 10)`).toBeTruthy();
        if (title.length > 70) expect.soft(false, `[SEO] Title too long on ${pagePath}: "${title}" (${title.length} chars, max 70)`).toBeTruthy();
      });
      ...
      await test.step('meta description 50–160 chars', async () => {
        ...
        if (!desc) { expect.soft(false, `[SEO] Missing meta description on ${pagePath}`).toBeTruthy(); return; }
        if (desc.length < 50) expect.soft(false, `[SEO] Meta description too short on ${pagePath} (${desc.length} chars, min 50)`).toBeTruthy();
        if (desc.length > 160) expect.soft(false, `[SEO] Meta description too long on ${pagePath} (${desc.length} chars, max 160)`).toBeTruthy();
      });
      ...
      await test.step('canonical URL is absolute', async () => {
        ...
        if (!canonical) { expect.soft(false, `[SEO] Missing canonical tag on ${pagePath}`).toBeTruthy(); return; }
        if (!/^https?:\/\//.test(canonical)) expect.soft(false, `[SEO] Canonical is not an absolute URL on ${pagePath}: "${canonical}"`).toBeTruthy();
      });
      ...
      await test.step('Open Graph tags present', async () => {
        ...
        if (!isArchive) {
          if (!ogTitle) expect.soft(false, `[SEO] Missing og:title on ${pagePath}`).toBeTruthy();
          if (!ogDesc) expect.soft(false, `[SEO] Missing og:description on ${pagePath}`).toBeTruthy();
          if (!ogImage) expect.soft(false, `[SEO] Missing og:image on ${pagePath}`).toBeTruthy();
          if (!ogUrl) expect.soft(false, `[SEO] Missing og:url on ${pagePath}`).toBeTruthy();
        } else {
      ...
      await test.step('exactly one H1', async () => {
        ...
        if (h1Count === 0) expect.soft(false, `[SEO] No H1 found on ${pagePath}`).toBeTruthy();
        if (h1Count > 1) expect.soft(false, `[SEO] Multiple H1 tags (${h1Count}) found on ${pagePath}`).toBeTruthy();
      });
      ...
      await test.step('all images have alt attributes', async () => {
        ...
        if (missingAlt.length > 0) {
          expect.soft(false, `[SEO] Images missing alt attribute on ${pagePath}:\n${missingAlt.map(s => `  - ${s}`).join('\n')}`).toBeTruthy();
        }
      });
      ...
      if (pagePath === '/') {
        await test.step('robots meta does not contain noindex', async () => {
          ...
          if (robots?.toLowerCase().includes('noindex')) {
            expect.soft(false, `[SEO] Homepage robots meta contains "noindex": "${robots}"`).toBeTruthy();
          }
        });
      }
    });
