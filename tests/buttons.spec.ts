...
import { BUTTONS } from '../test-utils/selectors';
...
test.describe('Buttons', () => {
  ...
  test('CTA buttons are visible on homepage @smoke', async ({ page }) => {
    const ctaButtons = page.locator(BUTTONS.CTA);
    const count = await ctaButtons.count();
    if (count === 0) {
      expect.soft(false, '[Buttons] No CTA buttons found on homepage').toBeTruthy();
      return;
    }
    for (let i = 0; i < count; i++) {
      const visible = await ctaButtons.nth(i).isVisible().catch(() => false);
      if (!visible)
        expect.soft(false, `[Buttons] CTA button ${i} is not visible on homepage`).toBeTruthy();
    }
  });
  ...
  test('CTA buttons have correct href actions @smoke', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!href) {
        expect.soft(false, `[Buttons] CTA button ${i} has no href`).toBeTruthy();
        continue;
      }
      if (!href.startsWith('http') && !href.startsWith('/')) {
        expect
          .soft(false, `[Buttons] CTA button ${i} href is not a valid URL: "${href}"`)
          .toBeTruthy();
      }
    }
  });
      ...
      try {
        ...
        if (currentPath !== expectedPath) {
          expect
            .soft(
              false,
              `[Buttons] CTA button ${i} navigated to "${currentPath}", expected "${expectedPath}"`
            )
            .toBeTruthy();
        }
      } catch {
        expect
          .soft(
            false,
            `[Buttons] CTA button ${i} click did not navigate to "${expectedPath}" within 10s`
          )
          .toBeTruthy();
      }
      await page.goto('/', { waitUntil: 'domcontentloaded' });
  ...
  test('CTA button text spelling is correct @smoke', async ({ page }) => {
    ...
    if (misspelled.length > 0) {
      expect
        .soft(
          false,
          `[Buttons] Spelling errors in CTA buttons:\n${misspelled.map((s) => `  - ${s}`).join('\n')}`
        )
        .toBeTruthy();
    }
  });
  ...
  test('buttons are enabled and not disabled @smoke', async ({ page }) => {
    const allButtons = page.locator(BUTTONS.ALL);
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (disabled !== null)
        expect.soft(false, `[Buttons] Button ${i} has disabled attribute`).toBeTruthy();
      if (ariaDisabled === 'true')
        expect.soft(false, `[Buttons] Button ${i} is aria-disabled`).toBeTruthy();
    }
  });
  ...
  test('buttons have visible and non-empty text @smoke', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const hasSvgOnly = await btn
        .evaluate((el) => {
          const children = [...el.childNodes];
          return children.every(
            (n) =>
              (n as Element).tagName === 'SVG' ||
      ...
      if (trimmed.length === 0) {
        expect
          .soft(
            false,
            `[Buttons] Button ${i} has no visible text (add text or aria-label)`
          )
          .toBeTruthy();
      }
    }
  ...
  test('upload field exists and is visible on contact page @smoke', async ({ page }) => {
    ...
    if (fileInputCount === 0) {
      console.warn('[Buttons] No file upload field found on contact page');
      return;
    }
    ...
    if (!parentVisible)
      expect
        .soft(false, '[Buttons] File upload input is not visible on contact page')
        .toBeTruthy();
  });
  ...
  test('upload trigger element is clickable on contact page @smoke', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (tag === 'input' && disabled !== null) {
        expect
          .soft(false, `[Buttons] Upload input ${i} is disabled on contact page`)
          .toBeTruthy();
      }
    }
  });
  ...
  test('footer contact button is clickable on all pages @smoke', async ({ page }) => {
    for (const pagePath of pagesToCheck.slice(0, 3)) {
      ...
      const contactBtn = page.locator(BUTTONS.FOOTER_CONTACT).first();
      ...
      if (disabled !== null)
        expect
          .soft(false, `[Buttons] Footer contact button is disabled on ${pagePath}`)
          .toBeTruthy();
    }
  });

  test('all buttons on page have consistent hover/focus state styling @smoke', async ({ page }) => {
    const buttons = page.locator(BUTTONS.CTA);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const focused = await buttons
        .nth(i)
        .evaluate((el) => el === document.activeElement)
        .catch(() => false);
      if (!focused)
        expect.soft(false, `[Buttons] Button ${i} is not keyboard focusable`).toBeTruthy();
    }
  });
});
